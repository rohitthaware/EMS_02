...
const Addtask = () => {
  ...
  const getProjects = () => {
    axios.get(url + '/admin/getprojects').then((response) => {
      ...
      if (result.status === 'success') {
        ...
        toast.error('error while loading list of Projects')
      }
    })
  }
  ...
  const getEmpByDept = () => {
    if (deptid.length === 0) {
      toast.info('Select Department Id')
    } else {
      ...
      axios.post(url + '/getemplist', data).then((response) => {
        ...
        if (result.status === 'success') {
          ...
          toast.error('error while loading list of Employee')
        }
      })
    }
  }
  ...
  const addUserToDB = () => {
    ...
    if (eDate < aDate) {
      toast.error('End Date should be Greater then Assign Date')
    } else if (pid.length === 0) {
      toast.error('Select Project id')
